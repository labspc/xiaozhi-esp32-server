...
| P1-STORAGE | 1 存储迁移 | EloqKV 影子迁移 + 校验脚本（全量/增量/比对）；回滚脚本 | feat/p1-storage-eloqkv-migrate | 待指派 | 待启动 | 依赖：EloqKV 实例可用；`ELOQKV_KEYS`、迁移骨架、`MIGRATION_RUNBOOK` 已备 |
| P2-RUST-WS | 2 Rust 迁移 | Axum WebSocket/HTTP 兼容旧路径 `/xiaozhi/*` + 新 `/orica/*`；PyO3 桥 | feat/p2-axum-ws-api | Codex | 进行中 | 原型含 Redis 读取、WS PyO3 stub、OpenAPI diff；待完善 AI 逻辑/认证 |
